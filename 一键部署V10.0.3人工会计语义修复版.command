...
echo "=================================================="
echo "WRITE Settlement Manager V10.0.3 人工会计语义修复版一键部署"
echo "Accounting Golden Compiler + Final Aggregation Hotfix"
echo "=================================================="
...
grep -q "version:'10.0.3'" src/release-v100.js || fail "src/release-v100.js 版本不是 10.0.3"
grep -q "const VERSION='10.0.3'" src/v10/export-runtime.js || fail "export-runtime 版本不是 10.0.3"
grep -q "const VERSION='10.0.3'" src/v10/runtime.js || fail "runtime 版本不是 10.0.3"
...
run "accounting golden aggregation" node docs/tests/v1003_accounting_golden_final_aggregation.js src/v10/billable-atom.js src/v10/production-core.js
run "V8.2 zero-loss" node docs/tests/v820_real_batch_zero_loss.js src/v8/semantic-core.js src/v8/zero-loss-engine.js docs/tests/V820_ZERO_LOSS_1001_1162_GOLDEN.json
run "V8.3 classification fidelity" node docs/tests/v830_classification_fidelity_golden.js src/v8/semantic-core.js src/v8/fidelity-evaluator.js docs/tests/V820_ZERO_LOSS_1001_1162_GOLDEN.json
run "V8.3.1 trace fidelity" node docs/tests/v831_trace_fidelity_golden.js src/v8/semantic-core.js src/v8/trace-fidelity.js docs/tests/V820_ZERO_LOSS_1001_1162_GOLDEN.json

echo "[4/10] Golden FACT 样式保护检查..."
# V10.0.3 必须动态使用模板真实 cellXfs 基线，禁止再硬编码 89。
...
TEMP_ROOT="$(mktemp -d /tmp/write-v1003.XXXXXX)" || fail "无法创建临时目录"
...
echo "[7/10] 覆盖 V10.0.3 修复文件并二次验收..."
...
if ! git diff --cached --quiet; then
  run "git commit" git commit -m "Deploy WRITE V10.0.3 accounting golden compiler hotfix"
...
echo "=================================================="
echo "✅ WRITE V10.0.3 最终修正版部署完成"
echo "✅ 版本检查已修复：不再误查 10.0.1"
echo "✅ FACT 最终按 origin + country + currency + role + complete configuration 唯一归并"

const V11013_PAGE_ERRORS=[];
function attachPageErrors(page){page.on('pageerror',e=>V11013_PAGE_ERRORS.push(String(e?.message||e)));return page}
...
const outDir=path.resolve(process.argv[4]||path.join(repo,'.v11013-x07-output'));
const profile=path.resolve(process.argv[5]||path.join(repo,'.v11013-browser-profile'));
...
(async()=>{
 ...
 let page=attachPageErrors(ctx.pages()[0]||await ctx.newPage());
 ...
 const page2=attachPageErrors(await ctx.newPage());await page2.goto(base,{waitUntil:'domcontentloaded'});await page2.waitForFunction(()=>window.WRITE_V106_SIMPLE_WORKFLOW,{timeout:30000});
 const snap1=await page.evaluate(()=>window.WRITE_V106_SIMPLE_WORKFLOW._test.manifestBegin('__v11013_tab_probe__.xlsx','SYNC',{observedHash:'old',localStatus:'LOCAL_SUCCESS',syncStatus:'SYNC_PENDING',expectedRuleIds:['old-rule']}));
 const snap2=await page2.evaluate(()=>window.WRITE_V106_SIMPLE_WORKFLOW._test.manifestBegin('__v11013_tab_probe__.xlsx','LEARN',{observedHash:'new',localStatus:'PENDING',syncStatus:'NOT_READY'}));
 const stale=await page.evaluate(s=>window.WRITE_V106_SIMPLE_WORKFLOW._test.manifestCas('__v11013_tab_probe__.xlsx',s,{syncStatus:'SYNCED'}),snap1);
 const fresh=await page2.evaluate(s=>window.WRITE_V106_SIMPLE_WORKFLOW._test.manifestCas('__v11013_tab_probe__.xlsx',s,{observedHash:'new',learnedHash:'new',learnedParserVersion:'11.0.12',learnedSchemaVersion:'5',localStatus:'LOCAL_SUCCESS',syncStatus:'SYNC_PENDING',expectedRuleIds:['new-rule']}),snap2);
 ...
 await ctx.route('**/api/rules/sync',route=>route.abort());
 page=attachPageErrors(ctx.pages()[0]||await ctx.newPage());
 ...
 const unknown=await page.evaluate(()=>{const r=window.WRITE_V10_PRODUCTION.build([{orderId:'X-UNKNOWN',trackingNumber:'X-TRACK',destinationCountry:'FRANCE',fulfillmentOrigin:'CN',currency:'EUR',lineItems:[{sku:'NEVER-LEARNED-11013',productName:'Never Learned Mystery Product',quantity:1}]}]);return r.rows?.[0]||null});
 ...
 await ctx.close();
 if(V11013_PAGE_ERRORS.length)throw new Error('Unexpected pageerror: '+V11013_PAGE_ERRORS.join(' | '));
console.log('V11.0.13 BROWSER X07 REPLAY/DOWNLOAD PASS',JSON.stringify({parcels:replay.parcels,reviewCount,total,pencil,exportPath:save}));

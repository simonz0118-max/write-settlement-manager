...
const soaps=x.rows.filter(r=>r.country==='FRANCE'&&/Savon/.test(r.description));
assert.equal(soaps.length,2,'CN/FR origins must remain separate');
assert(soaps.every(r=>r.quantity===1),'each origin keeps its own parcel quantity');
assert(soaps.every(r=>r.needsReview),'split shipment rows must be red');

...
for label,p in candidates:
 ...
 try:
  p.write_bytes(bak+b'\nV11012_NEGATIVE\n')
  rc=subprocess.run([sys.executable,'docs/tests/verify_release_manifest_v11012.py','.'],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL).returncode
  if rc==0:raise SystemExit(f'negative gate did not catch {label}')
 finally:p.write_bytes(bak)
subprocess.check_call([sys.executable,'docs/tests/verify_release_manifest_v11012.py','.'])
print('V11.0.12 MANIFEST NEGATIVE GATES PASS')

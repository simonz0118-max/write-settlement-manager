/* WRITE Settlement Manager v7.3.0 — legacy-first historical rule engine */
(function(){
'use strict';
...
async function beforeExport(){
 ...
 if(!audit.ok)throw new Error('V7.3 数据守恒未通过：'+audit.issues.map(x=>x.message).join('；'));

 // V7.3.0 is fully automatic by default.
 // Known historical patterns are priced by the history engine; truly unseen products are exported with blank prices.
 // No export-time modal is allowed to block the normal workflow.

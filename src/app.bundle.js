...
function startImport(fileList){
  ...
  worker?.terminate(); worker=new Worker('./src/workers/import.worker.bundle.js?v=7.1.6-20260810-0010'); importStartedAt=performance.now(); importedFileNames=files.map(f=>f.name);
...
// v7.1.6 — mandatory FACT delivery: generate a FACT when the source workbook has none.
...
async function buildGeneratedPencilFactWorkbook(workbookName){
  const resp=await fetch(`./assets/FACT_TEMPLATE_CN_CANONICAL_V1.xlsx?v=7.1.6`,{cache:'no-store'});
  ...
  // V7.1.6: release-time verified canonical path. This removes runtime XML-discovery as a point of failure.
...
async function buildGeneratedFactWorkbook(workbookName){
  ...
  const resp=await fetch(`./assets/FACT_TEMPLATE_LEARNED_V1.xlsx?v=7.1.6`,{cache:'no-store'});if(!resp.ok)throw new Error('无法读取内置 FACT 学习模板');
...
// v7.1.6 release notes controller — show once per release per browser
const WRITE_RELEASE_META = window.WRITE_RELEASE_META || {current:{version:document.body.dataset.release||'7.1.6',time:'',title:'WRITE Settlement Manager',sections:[]},history:[]};
const WRITE_RELEASE = {
  version: WRITE_RELEASE_META.current?.version || document.body.dataset.release || '7.1.6',
  date: WRITE_RELEASE_META.current?.time || '',
  title: `WRITE Settlement Manager v${WRITE_RELEASE_META.current?.version || document.body.dataset.release || '7.1.6'}`,
...
// v7.1.6 — version history from unified release metadata

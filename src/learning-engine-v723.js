/* WRITE Settlement Manager v7.2.3 — export policy / generic FACT flow */
(function(){
'use strict';
...
async function beforeExport(){
 ...
 return {ok:true,unknownCosts:unresolvedCostGroups().length,allowUnknownCosts:result.action==='skip'||unresolvedCostGroups().length>0,skipped:result.action==='skip',action:result.action};
}
function installReviewBatchButton(){
 ...
 const b=document.createElement('button');b.id='v722ReviewBatchButton';b.className='ghost-btn';b.textContent='批量设置分类 / 成本 / 币种';b.onclick=()=>batchRuleModal({forExport:false});head.appendChild(b);
}

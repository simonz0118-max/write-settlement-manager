/* WRITE Settlement Manager v7.4.0 — Universal FACT learning: post-export enhancement only */
(function(){
'use strict';
...
async function beforeExport(){
 ...
 // V7.4 universal contract:
 // only actual source-record loss / missing record identity can block export.
 // Product quantity mismatches, unknown categories and unknown prices are warnings only,
 // because the FACT must still preserve every available source field.
 const hard=audit.issues.filter(x=>x.type==='ORDER_RECORD_LOSS'||x.type==='MISSING_RECORD_KEY');
 if(hard.length)throw new Error('V7.4 源订单记录守恒未通过：'+hard.map(x=>x.message).join('；'));
 ...
 return {
   ...
   allowUnknownCosts:true,
   skipped:groups.length>0,
   action:groups.length?'AUTO_BLANK_PRICE':'AUTO_HISTORY',
   warnings:audit.issues.filter(x=>!hard.includes(x)),
   sourceDataFirst:true
 };
}

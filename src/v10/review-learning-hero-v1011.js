...
function renderFiles(p,rows){p.querySelector('#rlFiles').innerHTML=rows.map(x=>`<div class="rl-file ${x.ok?'ok':'bad'}"><i>${x.ok?'✓':'!'}</i><div><b>${esc(x.name)}</b><small>${x.ok?`${num(x.result.factRules)} FACT · ${num(x.result.productRules)} 商品 · ${num(x.result.costRules)} 成本`:esc(x.error)}</small></div></div>`).join('')}
...
async function syncCloud(){try{if(!g.WRITE_KB?.sync)throw Error('知识库未提供云同步接口');const detail=await g.WRITE_KB.sync({force:true});return{ok:true,msg:'Cloudflare D1 同步成功'+syncSummary(detail),detail:detail??null}}catch(e){return{ok:false,msg:'云端同步失败：'+(e?.message||e),detail:null}}}
...
async function run(files){
  ...
  const cloud=await syncCloud();view(t.failed?'批量学习完成（部分失败）':'批量学习完成',`源文件 ${raw.length} 个 · 实际 XLSX ${pack.expanded.length} 个`,100,false);renderSources(p,pack.sourceSummary);renderFiles(p,rows);

...
async function inflateRaw(a){const s=new Blob([a]).stream().pipeThrough(new DecompressionStream('deflate-raw'));return new Uint8Array(await new Response(s).arrayBuffer())}
async function readZip(blob){const b=new Uint8Array(await blob.arrayBuffer());let e=-1;for(let i=b.length-22;i>=Math.max(0,b.length-65557);i--){if(u32(b,i)===0x06054b50){e=i;break}}if(e<0)throw new Error('XLSX ZIP目录损坏');const n=u16(b,e+10);let p=u32(b,e+16);const out=new Map();for(let i=0;i<n;i++){if(u32(b,p)!==0x02014b50)throw new Error('ZIP central损坏');const m=u16(b,p+10),cs=u32(b,p+20),nl=u16(b,p+28),xl=u16(b,p+30),cl=u16(b,p+32),lo=u32(b,p+42),name=dec.decode(b.slice(p+46,p+46+nl));const lnl=u16(b,lo+26),lxl=u16(b,lo+28),raw=b.slice(lo+30+lnl+lxl,lo+30+lnl+lxl+cs);out.set(name,m===0?raw:m===8?await inflateRaw(raw):(()=>{throw new Error('不支持ZIP压缩方法')})());p+=46+nl+xl+cl}return out}
function writeZip(map){const parts=[],central=[];let off=0;for(const[name,data0]of map){const nameB=enc.encode(name),data=ArrayBuffer.isView(data0)?new Uint8Array(data0.buffer,data0.byteOffset,data0.byteLength):(data0 instanceof ArrayBuffer?new Uint8Array(data0):enc.encode(String(data0))),crc=crc32(data),local=new Uint8Array([...p32(0x04034b50),...p16(20),...p16(0x800),...p16(0),...p16(0),...p16(0),...p32(crc),...p32(data.length),...p32(data.length),...p16(nameB.length),...p16(0),...nameB]);parts.push(local,data);central.push(new Uint8Array([...p32(0x02014b50),...p16(20),...p16(20),...p16(0x800),...p16(0),...p16(0),...p16(0),...p32(crc),...p32(data.length),...p32(data.length),...p16(nameB.length),...p16(0),...p16(0),...p16(0),...p16(0),...p32(0),...p32(off),...nameB]));off+=local.length+data.length}const cs=central.reduce((s,x)=>s+x.length,0),co=off;return new Blob([...parts,...central,new Uint8Array([...p32(0x06054b50),...p16(0),...p16(0),...p16(map.size),...p16(map.size),...p32(cs),...p32(co),...p16(0)])],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'})}
function txt(m,n){const b=m.get(n);if(!b)throw new Error('XLSX缺少 '+n);return dec.decode(b)}function setTxt(m,n,v){m.set(n,enc.encode(v))}
function sheets(x){return[...x.matchAll(/<sheet\b([^>]*)\/>/g)].map(m=>({name:/name="([^"]+)"/.exec(m[1])?.[1]||'Sheet',id:+(/sheetId="(\d+)"/.exec(m[1])?.[1]||0),rid:/r:id="([^"]+)"/.exec(m[1])?.[1]||''}))}
function rels(x){const out=new Map();for(const m of x.matchAll(/<Relationship\b([^>]*)\/?\s*>/g)){const a=m[1],id=/\bId="([^"]+)"/.exec(a)?.[1],target=/\bTarget="([^"]+)"/.exec(a)?.[1];if(id&&target)out.set(id,target)}return out}
function col(n){let s='';while(n){n--;s=String.fromCharCode(65+n%26)+s;n=Math.floor(n/26)}return s}
function cell(ref,v){if(v==null||v==='')return`<c r="${ref}"/>`;if(typeof v==='number'&&Number.isFinite(v))return`<c r="${ref}"><v>${v}</v></c>`;return`<c r="${ref}" t="inlineStr"><is><t xml:space="preserve">${esc(v)}</t></is></c>`}
...
async function addLearningSource(factBlob,res,wbName){
 const fact=await readZip(factBlob);let w=txt(fact,'xl/workbook.xml'),wr=txt(fact,'xl/_rels/workbook.xml.rels'),ct=txt(fact,'[Content_Types].xml');
 if(sheets(w).some(s=>s.name==='WRITE_LEARNING_SOURCE'))return writeZip(fact);
 let sid=Math.max(...sheets(w).map(x=>x.id),0)+1,rid=Math.max(...[...wr.matchAll(/Id="rId(\d+)"/g)].map(x=>+x[1]),0)+1,sn=Math.max(...[...fact.keys()].map(n=>+(/sheet(\d+)\.xml/.exec(n)?.[1]||0)),1)+1;
 const path=`xl/worksheets/sheet${sn}.xml`,r=`rId${rid}`;fact.set(path,enc.encode(simpleSheet(learningRows(res,wbName))));
 w=w.replace('</sheets>',`<sheet xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" name="WRITE_LEARNING_SOURCE" sheetId="${sid}" state="veryHidden" r:id="${r}"/></sheets>`);
 wr=wr.replace('</Relationships>',`<Relationship Id="${r}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/${path.split('/').pop()}"/></Relationships>`);
 ...
 setTxt(fact,'xl/workbook.xml',w);setTxt(fact,'xl/_rels/workbook.xml.rels',wr);setTxt(fact,'[Content_Types].xml',ct);return writeZip(fact)
}
...
g.WRITE_V101_WORKBOOK={VERSION,readZip,writeZip,sheets,rels,simpleSheet,learningRows,addLearningSource,mergeFactAndAccounting};
})(window);
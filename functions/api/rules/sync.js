...
function clean(r={}){
  return {
    ruleId:String(r.ruleId||''),type:String(r.type||'GENERIC'),lookupKey:String(r.lookupKey||''),
    ...
    version:Number(r.version)||1,syncState:'SYNCED',deleted:!!r.deleted
  };
}
async function writeRule(db,r){
  const x=clean(r); if(!x.ruleId||!x.lookupKey)return false;
  const old=await db.prepare(`SELECT updated_at,version FROM ${TABLE} WHERE rule_id=?`).bind(x.ruleId).first();
  if(old && String(old.updated_at||'')>x.updatedAt)return true;
  await db.prepare(`INSERT INTO ${TABLE}
    ...
    .bind(x.ruleId,x.type,x.lookupKey,JSON.stringify(x.payload),x.confidenceLevel,x.priority,x.source,x.confirmed?1:0,
          x.deviceId,x.createdAt,x.updatedAt,x.version,'SYNCED',x.deleted?1:0).run();
  return true;
}
function fromRow(r){
...
export async function onRequestGet({env}){
  ...
  return json({ok:true,service:'WRITE_RULES_SYNC',version:'10.1.7',method:'GET'});
}
export async function onRequestPost({request,env}){
  try{
    ...
    const accepted=[];
    for(const r of rules){ if(await writeRule(db,r))accepted.push(String(r.ruleId||'')); }
    const since=body.since?String(body.since):null;
    const q=since
      ? db.prepare(`SELECT * FROM ${TABLE} WHERE updated_at>? ORDER BY updated_at ASC LIMIT 10000`).bind(since)
      : db.prepare(`SELECT * FROM ${TABLE} ORDER BY updated_at ASC LIMIT 10000`);
    const out=await q.all(), rows=Array.isArray(out.results)?out.results:[];
    const cursor=rows.length?String(rows[rows.length-1].updated_at):new Date().toISOString();
    return json({ok:true,acceptedRuleIds:accepted,rules:rows.map(fromRow),cursor,serverVersion:'10.1.7'});

## V11.2.0 — Folder Automation

- 浏览器打开期间自动扫描审核文件夹并静默学习。
- SHA-256 增量识别、CAS/路径锁、多标签串行、断网/401重试。
- 常驻状态中心；权限失效只等待一次用户重新授权。
- 冻结定价核心和 FACT 付款结构不变。
...
- 全 JS syntax + 浏览器 pageerror fail-closed。
- FACT!B22 真实付款格及五类负向门禁。
- 只做 V11.0.12→V11.0.13 增量，不改冻结定价核心/FACT模板/付款计算。
...
- 修复设置菜单字号与标准齿轮图标。
- 重建首页/导入后 Hero 的上传、文件、数据库图标并按真实按钮语义绑定。
- 保留 V10.6.15 文案与 V10.6.14 Settings + Earth UI。
- V10.6.6 导出权威与 V10.5.4 冻结核心不修改。

## V10.6.15 — Hero Copy Update

- 首页副标题改为：导入订单、云端AI分类统计、自我学习进化。越用越聪明的系统。
- 修改 V10.6.12 权威 Hero 文案源，并提升缓存版本号，避免旧文案回写或缓存。
- V10.6.14 设置/按钮动效/Earth UI 保持不变。
- V10.6.6 导出权威与 V10.5.4 冻结核心不修改。

## V10.6.14 — Settings + Earth UI

- 设置中可修改默认人工审核文件夹。
- 所有按钮增加统一点击反馈。
- 主视觉改为石墨黑 / 暖白 / 铜棕 / 橄榄 / 陶土，不再使用蓝紫主色。
- V10.6.6 导出权威与 V10.5.4 冻结核心不修改。
...
- 展开侧边栏严格统一 24px 图标列、46px 行高、文本/徽标对齐；折叠仍只显示菜单按钮。
- 所有正式导出入口强制先显示“仅 FACT / FACT + 其他页面”选择，再把明确 mode 传给 V10.6.0 导出引擎。
- 首页/结果页“导入审核后数据”强制走默认审核文件夹，不再触发旧文件选择器；首次选文件夹，之后自动扫描变化并学习。
- 根据用户提供的 2026-09-04 实际导出 XLSX，确认此前导出包含 5 个可见业务 Sheet + veryHidden 学习溯源，本版封闭该入口绕行。
- V10.5.4 冻结定价、学习语义与 D1 核心保持不变。

## V10.6.1 — Professional Hero UI

- 折叠侧边栏只保留菜单按钮，所有导航图标完全隐藏。
- Hero 首页恢复清晰品牌标题和说明，三项核心操作升级为大尺寸专业卡片。
- 优化桌面/iPad/iPhone 响应式布局、间距、字号、Hover 和点击区域。
- 保留 V10.6.0 FACT 默认导出与审核文件夹工作流。
- V10.5.4 冻结定价/学习/D1 核心不变。
